...
const headCells: readonly { id: keyof Data; numeric: boolean; disablePadding: boolean; label: string }[] = [
  ...
  { id: 'accommodation', numeric: false, disablePadding: false, label: 'Bottle Code' },
  { id: 'email', numeric: false, disablePadding: false, label: 'UserId' },
];
...
export default function Orderss() {
  const { data: rows = [], isLoading } = useQuery("fetchMyshops", apiClient.fetchShop, {
    onError: () => {},
  });
  ...
  const { mutate: mutateDelete } = useMutation(apiClient.fetchshopByIddelete, {
    onSuccess: () => {
      showToast({ message: "Successfully deleted!", type: "SUCCESS" });
      queryClient.invalidateQueries("fetchMyshops");
      navigate('/tables')
    },
  });
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedUser = await apiClient.fetchUserById(id);
        setUser(fetchedUser);
